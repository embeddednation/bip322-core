...
and uses this package as a library. Everything that talks to a node is there.
...
* `bip322-refcheck corpus`: 46 signatures judged by this tool, btclib, btcd
  ...
  to this tool's.
* The audit workflow's tests (against a fake node and against a real regtest
  Bitcoin Core) live in the bip322-audit repository.
