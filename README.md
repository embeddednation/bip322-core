# bip322ms — BIP-322 message signing for a P2WSH multisig quorum
...
| Verifier | Origin |
|---|---|
| ours | `bip322ms.verify` with btclib + libbitcoinkernel |
...
* embit's stock `PSBT` rebuilds the unsigned transaction with `version or 2`
  ...
  `bip322ms.psbt.BIP322PSBT` overrides both. Do not round-trip these PSBTs
...
bip322ms/cli.py       bip322ms command
